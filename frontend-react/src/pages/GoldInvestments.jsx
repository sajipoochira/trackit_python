...
export default function GoldInvestments(){
  ...
  const fetchAll = React.useCallback(async () => {
    ...
    // From Investments (category Gold)
    inv.filter(x => (String(x.category || '')).toLowerCase() === 'gold').forEach(x => {
      addBuy(x.name || 'GOLD', x.quantity, x.buy_price, parseChargesFromNotes(x.notes), x.currency)
    })
    // Expense fallback: "Gold Buy <NAME>"
    exp.filter(x => (x.title || '').toLowerCase().startsWith('gold buy')).forEach(x => {
      const t = String(x.title || '')
      const m = t.match(/gold buy\s+(.+)/i)
      const name = (m ? m[1] : 'GOLD').toUpperCase()
    ...
    // Income fallback: "Gold Sell <NAME>"
    inc.filter(x => (x.title || '').toLowerCase().startsWith('gold sell')).forEach(x => {
      const t = String(x.title || '')
      const m = t.match(/gold sell\s+(.+)/i)
      const name = (m ? m[1] : 'GOLD').toUpperCase()
      const qty = parseQtyFromNotes(x.notes)
      addSell(name, qty)
    })
  ...
  return (
    <div>
      <h2 className="h4 mb-3">Gold Investments</h2>
      <div className="card mb-3">
        <div className="card-header"><strong>Current Gold Holdings</strong></div>
...
function BuyCard(){
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await api.createInvestment({ name, quantity: Number(qty), buy_price: Number(price), currency, date, category: 'Gold', notes: `Charges: ${charges}; Total: ${total}` })
      setMsg('Buy recorded under Investments')
      setName(''); setQty(''); setPrice(''); setCharges('0'); setDate('')
    } catch (e1) {
      try {
        await api.createExpense({ title: `Gold Buy ${name}`, amount: total, currency, date, notes: `Qty:${qty} @ ${price}; Charges:${charges}` })
  ...
  return (
    <div className="card mb-3">
      <div className="card-header"><strong>Buy Gold</strong></div>
      <div className="card-body">
        {msg && <div className="alert alert-success">{msg}</div>}
        {err && <div className="alert alert-danger">{err}</div>}
        <form onSubmit={onSubmit}>
          <div className="row g-3">
            <div className="col-md-4"><label className="form-label">Item/Name</label><input className="form-control" value={name} onChange={e=>setName(e.target.value)} required /></div>
...
function SellCard(){
  ...
  const onSubmit = async (e) => {
    ...
    setMsg(''); setErr(''); setSaving(true)
    try {
      await api.createIncome({ title: `Gold Sell ${name}`, amount: proceeds, currency, date, notes: `Qty:${qty} @ ${price}; Charges:${charges}` })
  ...
  return (
    <div className="card mb-3">
      <div className="card-header"><strong>Sell Gold</strong></div>
      <div className="card-body">
        {msg && <div className="alert alert-success">{msg}</div>}
        {err && <div className="alert alert-danger">{err}</div>}
        <form onSubmit={onSubmit}>
