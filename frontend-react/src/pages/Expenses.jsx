...
export default function Expenses() {
  ...
  useEffect(() => { (async () => { setFormConfig(await getFormConfig('/expenses/')); await load() })() }, [])

  const useConfig = React.useMemo(()=>pickExpenseConfig(formConfig), [formConfig])
  ...
  async function onDelete(item) {
    ...
    try { await api.deleteExpense(id); await load() } catch (e) { alert(e?.message || 'Delete failed') }
  }

  async function handleSubmit(payload) {
    if (formMode==='create') await api.createExpense(payload)
    else if (formMode==='edit' && editingId) await api.updateExpense(editingId, payload)
    setFormOpen(false)
    await load()
  }
  ...
  return (
    <div>
      ...
      {formOpen && (
        <DynamicForm
          title={`${formMode==='create'?'Create':'Edit'} Expense`}
      ...
      {bulkOpen && (
        <BulkCreateCard
          ...
          text={bulkText}
          setText={setBulkText}
          onSubmit={async (item)=>{ await api.createExpense(item) }}
      ...
      <div className="card">
        <div className="card-header"><strong>Expense List</strong></div>
...
function pickExpenseConfig(config){
  if ((config.order && config.order.length) || (config.overrides && Object.keys(config.overrides).length)) return config
  return {
    order: ['title','category','amount','currency','date','notes'],
    overrides: {
      title: { type: 'text', required: true, label: 'Expense Title' },
      category: { type: 'select', required: true, options: ['Food and Groceries','Transportation','Bills & Utilities','Healthcare','Entertainment','Shopping','Rent','Fuel','Other'] },
      amount: { type: 'number', required: true, step: 0.01, min: 0 },
      currency: { type: 'select', options: ['INR','QAR'], required: true },
      date: { type: 'date', required: true },
      notes: { type: 'text', required: false }
    },
    template: { title: '', category: '', amount: 0, currency: 'INR', date: '', notes: '' },
